...
public class addoAsyncStreams {
	...
	private class AIN implements Runnable {
				...
				while (working) {
					...
					if (!prefix) {
						...
						if (msgLength == STREAM_PREFIX) {
							...
							ba.raiseEventFromDifferentThread(addoAsyncStreams.this, null, 0, eventName + "_newstream", true, new Object[] {StreamFolder, FileName});
							continue;//!!!
						} else {
					...
					ba.raiseEventFromDifferentThread(addoAsyncStreams.this, null, 0, ev, true, new Object[] {data});
				}
		...
		private void closeUnexpected() throws IOException {
			ba.raiseEventFromDifferentThread(addoAsyncStreams.this, null, 0, eventName + "_terminated", false, null);
			addoAsyncStreams.this.Close(); //close both threads
		}
		public void close()  {
			working = false;
			try {
